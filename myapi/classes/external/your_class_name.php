...
class your_class_name extends external_api
{
     ...
     public static function get_data($param)
     {
          ...
          $params = self::validate_parameters(self::get_data_parameters(), ['param' => $param]);

          $data = $DB->get_records_sql('SELECT * FROM {table_name} WHERE field = ?', [$param]);
          ...
          return new external_single_structure([
               'data' => new external_multiple_structure( // Use external_multiple_structure here
                    new external_single_structure([
                         'fieldname' => new external_value(PARAM_TEXT, 'Field description')
                    ])
               )
          ]);
     }
     ...
     public static function get_students_enrolled_by_password($courseid)
     {
          ...
          // Validate parameters
          $params = self::validate_parameters(self::get_grade_categories_parameters(), ['courseid' => $courseid]);

          // Fetch students enrolled by self-enrollment with password
